 ...
 * These 'kazen' can be generated with a 'kaas' functions that accepts a predicate as first argument and a function as
 * second argument.
 * The 'kaas' function closes over the predicate and the second argument and gives an optional result to be used with
 * the beforementioned match function.
 *
 * Several overloads exists for the 'kaas' and 'match' functions for your convenience.
 *
 * Accidently 'kaas' is the Dutch word for cheese with plural 'kazen'. 'Kaas' is use the to prevent a name clash with
 * the 'case' keyword.
 * Leipajuusto, if fitted with the proper diacritics, is a Swedish/Finish word for a cheese served with coffee. 'Java'
 * is also American slang for coffee.
...
public final class CaseFactory {
   ...
   * 
   * @param clazz
   *          the class that the value of the argument of the returned function should be assignable from.
   * @param f
   *          the function to execute when clazz is assignable from the given argument of the returned function.
   ...
   * 
   */
  public static <T, R> Case<T, R> kaas(final Class<T> clazz, final R value) {
   ...
   *          the class that the value of the argument of the returned function should be assignable from.
   * @param f
   *          the function to execute when clazz is assignable from the given argument of the returning function.
   ...
   * 
   */
  public static <T1, T2, R> Case<T2, R> kaas(final Class<T1> clazz, final BiFunction<Class<T1>, T2, R> f) {
    return (T2 t) -> t == null || clazz.isAssignableFrom(t.getClass()) ? Optional.of(f.apply(clazz, t))
        : Optional.empty();
  }
   ...
   * 
   */
  public static <T1, T2, R> Case<T2, R> kaas(final Class<T1> clazz, final Function<T2, R> f) {
   ...
   *          the predicate
   * @param f
   *          the function to execute when p is truthy.
   * @return the Optional result of f if p is truthy. Return Optional.Empty() if p is falsy.
   */
  public static <T1, T2, R> Case<T1, R> kaas(final Function<T1, Optional<T2>> p, final BiFunction<T1, T2, R> f) {
    return (T1 t) -> {
      final Optional<T2> pOptionalResult = p.apply(t);
   ...
   * @return the Optional result of f if p is truthy. Return Optional.Empty() if p is falsy.
   */
  public static <T1, T2, R> Case<T1, R> kaas(final Function<T1, Optional<T2>> p, final Function<T2, R> f) {
    return (T1 t) -> {
      final Optional<T2> pOptionalResult = p.apply(t);
  ...
  public static <T1, T2, R> Case<T1, R> kaas(final Function<T1, Optional<T2>> p, final R value) {
    return (T1 t) -> {
      final Optional<T2> pOptionalResult = p.apply(t);
   ...
   *          the predicate
   * @param f
   *          the function to execute when p is true
   * @return the Optional result of f if p is true. Return Optional.Empty() if p is false
   */
  public static <T, R> Case<T, R> kaas(final Predicate<T> p, final Function<T, R> f) {
   ...
   */
  public static <T, R> Case<T, R> kaas(final Predicate<T> p, final R value) {
   ...
   * 
   * @param t1
   *          value to test for equality with t
   * @param f
   *          the function to execute when p is true
   * @return the Optional result of f if p is true. Return Optional.Empty() if p is false
   */
  public static <T, R> Case<T, R> kaasValue(final T t1, final Function<T, R> f) {
   ...
   * 
   * @param v
   *          the value to test for equality with t
   * @param f
   *          the function to execute when p is true
   * @return the Optional result of value if p is true. Return Optional.Empty() if p is false
   */
  public static <T, R> Case<T, R> kaasValue(T t1, final R value) {
   ...
   * 
   * @param value
   *          to return for every t.
   * @return Optional.of(value)
   */
  public static <T, R> Case<T, R> orelse(final R value) {
    return (T t) -> {
      return Optional.of(value);
    };
  }
